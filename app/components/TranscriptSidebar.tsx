...
export default function TranscriptSidebar({ transcript, feedEndRef }: TranscriptSidebarProps) {
  return (
    <section className="w-[280px] shrink-0 flex flex-col border border-white/[0.04] bg-[#0c0f1d]/20 rounded-xl p-3 min-h-0">
      <div className="flex items-center gap-2 text-[9px] uppercase tracking-[0.24em] text-zinc-500 pb-2 border-b border-white/[0.04] mb-2">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1 text-[11px] font-mono scroll-smooth">
        {transcript.length === 0 ? (
          ...
          transcript.map((line, index) => (
            <div key={`${line.agent}-${index}`} className="border-b border-white/[0.02] pb-2 last:border-0">
